...
/// <summary>Cauchy distribution (Lorentz distribution).
/// Heavy-tailed distribution used in robust statistics.
/// PDF(x) = 1/(πγ(1 + ((x-x₀)/γ)²))
/// </summary>module CauchyDistribution =
    
    /// <summary>Probability density function.
/// </summary>let pdf (x0: float) (gamma: float) (x: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
...
/// </summary>let cdf (x0: float) (gamma: float) (x: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
...
/// </summary>let ppf (x0: float) (gamma: float) (p: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
...
/// </summary>let rvs (x0: float) (gamma: float) (seed: int option) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
    ...
    /// <summary>Median: x₀
/// </summary>let median (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
            return x0
    ...
    /// <summary>Mode: x₀
/// </summary>let mode (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
            return x0
    ...
    /// <summary>Mean: undefined (infinite)
/// Cauchy distribution has no finite mean.
/// </summary>let mean (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
    ...
    /// <summary>Variance: undefined (infinite)
/// Cauchy distribution has no finite variance.
/// </summary>let variance (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
    ...
    /// <summary>Standard deviation: undefined
/// </summary>let std (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
    ...
    /// <summary>Entropy: ln(4πγ)
/// </summary>let entropy (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
    ...
    /// <summary>Interquartile range: 2γ
/// Distance between 25th and 75th percentiles.
/// </summary>let iqr (x0: float) (gamma: float) : FowlResult<float> =
        result {
            if gamma <= 0.0 then
                return! Error.invalidArgument "Cauchy gamma must be positive"
            // Q3 - Q1 = (x₀ + γ) - (x₀ - γ) = 2γ
